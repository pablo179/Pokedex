...
function PokemonCard({
  ...
  const getDataArray = async () => {
    ...
    let locationArea; let area; const
      list = [];
    ...
    for (let i = 0; i < dataRequest.data.stats.length; i += 1) {
  ...
  return (
    <Card color={color}>
      ...
      <DataRow>
        ...
        <TypeContainer>
          {
            types.map((item) => typeImages[item.type.name] && (
              <TypeImage src={typeImages[item.type.name]} />
            ))
          }
        </TypeContainer>
        <Information>
